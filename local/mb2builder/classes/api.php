	...
	class mb2builderApi
	{
		 ...
		 */
		 ...
		 /*
		  ...
		  public static function get_element_templates ()
		  {
			  ...
			  foreach ( $elements as $e )
			  {
				  $output .= format_text('[mb2pb_' . $e . ' template="1"][/mb2pb_' . $e . ']');
			  }
		...
		public static function get_images_preview()
		{
			...
			foreach( $files as $f )
			{
				...
				$output .= '<a href="#" class="mb2-pb-insert-image" data-imgurl="' . $url . '" data-imgname="' . $f->get_filename() . '" data-dismiss="modal">';
		...
		public static function modal_template( $type = '', $opts = array() )
		{
			...
			if ( $type === 'images' || $type === 'font-icons' || $type === 'elements' || $type === 'import-export' )
			{
				$modal_cls = ' modal-md';
				$modalcls = '';
			}
		...
		public static function import_export( $opts = array() )
		{
			...
			$output .= '<ul class="nav nav-tabs">';
			$output .= '<li class="nav-item active"><a class="nav-link active show" data-toggle="tab" href="#tab-importrows">' . get_string('importrows','local_mb2builder') . '</a></li>';
			$output .= '<li class="nav-item"><a class="nav-link" data-toggle="tab" href="#tab-importtemplates">' .
			get_string('layouts','local_mb2builder') . '</a></li>';
			...
			$output .= '<div id="tab-importrows" class="tab-pane fade in active">';
			$output .= self::get_import_blocks_list();
			$output .= '</div>';

			$output .= '<div id="tab-importtemplates" class="tab-pane fade">';
